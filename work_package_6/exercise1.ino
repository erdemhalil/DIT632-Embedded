...
//Use a motor with the setting RPM 45!! See the corrected Work Package description.
#define ENCA 2
#define ENCB 3
#define PWM1 5
#define PWM2 6
...
int kp = 20; // Proportional constant for controller (tuning parameter)
...
void setup() {
  ...
  */
  ...
  // attach an interrupt to channel a for readEncoder function whenever the pin goes from LOW to HIGH
...
int getAction(int error){
  ...
  */
  ...
  // derive a control signal using error and kp as per assigment description
  u_out = error * kp;
...
void ISR_readEncoder(){
  ...
  */
  ...
  // Read signal from both encoders
  a = digitalRead(ENCA);
  b = digitalRead(ENCB);
  
  // If signal A is on, increase position
  if(a > b)
  {
    pos++;
  } 
  else // if Signal B is on, decrease position
  {
    pos--;
  }
}